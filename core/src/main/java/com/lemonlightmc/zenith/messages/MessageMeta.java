...
import com.lemonlightmc.zenith.messages.IMessageSource.IMessageMeta;
...
public class MessageMeta implements IMessageMeta {
  ...
  public void modify(final ResourceBundle bundle) {
    if (bundle.containsKey("meta.locale")) {
      this.locale = StringUtils.parseLocale(ResourceUtils.getResourceBundleString(bundle, "meta.locale", "en"));
    }
    if (bundle.containsKey("meta.version")) {
      this.version = new Version(ResourceUtils.getResourceBundleString(bundle, "meta.version", "1.0.0"));
    }
    if (bundle.containsKey("meta.author")) {
      this.author = ResourceUtils.getResourceBundleString(bundle, "meta.author", "Unknown");
    }
    if (bundle.containsKey("meta.description")) {
      this.description = ResourceUtils.getResourceBundleString(bundle, "meta.description", "No description");
    }
  }
